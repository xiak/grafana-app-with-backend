...
func TestDecodeValues(t *testing.T) {
	form, err := url.ParseQuery("a=19&age=18&b=true&bool=false&byte=MTIz&bytes=MTIz&count=3&d=22.22&double=12.33&duration=" +
		...
		"simples=5566&string=go-xiak&timestamp=1970-01-01T00%3A00%3A20.000000002Z&uint32=32&uint64=64&very_simple.component=5566")
	if err != nil {
		t.Fatal(err)
	}
...
func TestPopulateMapField(t *testing.T) {
	query, err := url.ParseQuery("map%5Bxiak%5D=https://go-xiak.dev/")
	if err != nil {
		t.Fatal(err)
	}
	comp := &complex.Complex{}
	...
	err = populateMapField(field, comp.ProtoReflect().Mutable(field).Map(), []string{"map[xiak]"}, query["map[xiak]"])
	if err != nil {
		t.Fatal(err)
	}
	// Get the comp map field value
	if query["map[xiak]"][0] != comp.Map["xiak"] {
		t.Errorf("want: %s, got: %s", query["map[xiak]"], comp.Map["xiak"])
	}
}

func TestPopulateMapSepField(t *testing.T) {
	query, err := url.ParseQuery("map.name=xiak")
	if err != nil {
		t.Fatal(err)
	}
	comp := &complex.Complex{}
...
func TestParseURLQueryMapKey(t *testing.T) {
	tests := []struct {
		...
		{
			fieldName: "map[xiak]", field: "map", fieldKey: "xiak", err: nil,
		},
		{
		...
		{
			fieldName: "map[xiak]=", field: "", fieldKey: "", err: errInvalidFormatMapKey,
		},
		{
			fieldName: "[xiak]", field: "", fieldKey: "", err: errInvalidFormatMapKey,
		},
		{
