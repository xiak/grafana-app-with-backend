...
import (
	...
	bdtest "github.com/xiak/grafana-app-with-backend/pkg/internal/testdata/binding"
	"github.com/xiak/grafana-app-with-backend/pkg/internal/testdata/complex"
	ectest "github.com/xiak/grafana-app-with-backend/pkg/internal/testdata/encoding"

	"github.com/xiak/grafana-app-with-backend/pkg/common/encoding"
)

// This variable can be replaced with -ldflags like below:
// go test "-ldflags=-X github.com/go-xiak/xiak/v2/encoding/form.tagNameTest=form"
...
func TestFormCodecMarshal(t *testing.T) {
	...
	if !reflect.DeepEqual([]byte("password=xiak_pwd&username=xiak"), content) {
		t.Errorf("expect %s, got %s", "password=xiak_pwd&username=xiak", content)
	}
	...
	if !reflect.DeepEqual([]byte("username=xiak"), content) {
		t.Errorf("expect %s, got %s", "username=xiak", content)
	}
	...
	if !reflect.DeepEqual([]byte("id=1&name=xiak"), content) {
		t.Errorf("expect %s, got %s", "id=1&name=xiak", content)
	}
}
...
func TestFormCodecUnmarshal(t *testing.T) {
	...
	if !reflect.DeepEqual("xiak", bindReq.Username) {
		t.Errorf("expect %v, got %v", "xiak", bindReq.Username)
	}
	if !reflect.DeepEqual("xiak_pwd", bindReq.Password) {
		t.Errorf("expect %v, got %v", "xiak_pwd", bindReq.Password)
	}
}
...
func TestProtoEncodeDecode(t *testing.T) {
	in := &complex.Complex{
		...
		D:       22.22,
		Byte:    []byte("123"),
		Map:     map[string]string{"xiak": "https://go-xiak.dev/", "xiak_start": "https://go-xiak.dev/en/docs/getting-started/start/"},
		...
		String_:   &wrapperspb.StringValue{Value: "go-xiak"},
	...
	if "a=19&age=18&b=true&bool=false&byte=MTIz&bytes=MTIz&count=3&d=22.22&double=12.33&duration="+
		...
		"map%5Bxiak%5D=https%3A%2F%2Fgo-xiak.dev%2F&map%5Bxiak_start%5D=https%3A%2F%2Fgo-xiak.dev%2Fen%2Fdocs%2Fgetting-started%2Fstart%2F&"+
		"numberOne=2233&price=11.23&sex=woman&simples=3344&simples=5566&string=go-xiak"+
...
func TestDecodeStructPb(t *testing.T) {
	...
	query := `data={"name":"xiak"}&data_list={"name1": "xiak"}&data_list={"name2": "go-xiak"}`
	if err := encoding.GetCodec(Name).Unmarshal([]byte(query), req); err != nil {
		t.Fatal(err)
	}
	if "xiak" != req.Data.GetFields()["name"].GetStringValue() {
		t.Errorf("except %v, got %v", "xiak", req.Data.GetFields()["name"].GetStringValue())
	}
	if len(req.DataList) != 2 {
		t.Fatalf("execpt %v, got %v", 2, len(req.DataList))
	}
	if "xiak" != req.DataList[0].GetFields()["name1"].GetStringValue() {
		t.Errorf("except %v, got %v", "xiak", req.Data.GetFields()["name1"].GetStringValue())
	}
	if "go-xiak" != req.DataList[1].GetFields()["name2"].GetStringValue() {
		t.Errorf("except %v, got %v", "go-xiak", req.Data.GetFields()["name2"].GetStringValue())
	}
}
