...
func TestEnvWithPrefix(t *testing.T) {
	...
	// set env
	prefix1, prefix2 := "XIAK_", "FOO"
	envs := map[string]string{
		prefix1 + "SERVICE_NAME": "xiak_app",
...
func Test_env_load(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: []*config.KeyValue{
				{Key: "SERVICE_NAME", Value: []byte("xiak_app"), Format: ""},
...

		{
			...
			args: args{
				envStrings: []string{
					"__SERVICE_NAME=xiak_app",
					"__ADDR=192.168.0.1",
					"__AGE=20",
				},
			},
			want: []*config.KeyValue{
				{Key: "_SERVICE_NAME", Value: []byte("xiak_app"), Format: ""},
...

		{
			name: "with prefixes",
			fields: fields{
				prefixes: []string{"xiak_", "FOO"},
			},
			args: args{
				envStrings: []string{
					"xiak_SERVICE_NAME=xiak_app",
					"xiak_ADDR=192.168.0.1",
					"FOO_AGE=20",
				},
			},
			want: []*config.KeyValue{
