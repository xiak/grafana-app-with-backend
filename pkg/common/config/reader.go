...
func (r *reader) Merge(kvs ...*KeyValue) error {
	...
	for _, kv := range kvs {
		...
		if err := r.opts.decoder(kv, next); err != nil {
			log.DefaultLogger.Error("Failed to config decode error: ", err, " key: ", kv.Key, " value: ", string(kv.Value))
			return err
		}
		if err := r.opts.merge(&merged, convertMap(next)); err != nil {
			log.DefaultLogger.Error("Failed to config merge error: ", err, " key: ", kv.Key, " value: ", string(kv.Value))
			return err
		}
	}
	r.lock.Lock()
